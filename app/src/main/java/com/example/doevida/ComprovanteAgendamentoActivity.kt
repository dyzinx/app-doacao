            ...
            .addOnSuccessListener { doc ->
                ...
                val resumo = buildString {
                    ...
                    questionario?.forEach { (key, value) ->
                        val titulo = when (key.toString()) {
                            "Tipo Sanguíneo" -> "Tipo Sanguíneo"
                            "Gênero" -> "Gênero"
                            "Estado Civil" -> "Estado Civil"
                            "Nacionalidade" -> "Nacionalidade"
                            "Possui Alergias" -> "Possui Alergias"
                            "Tem Doença Crônica" -> "Doença Crônica"
                            "Vacinado" -> "Vacinado"
                            "Fuma" -> "Fuma"
                            "Bebidas Alcoólicas" -> "Bebidas Alcoólicas"
                            "Alergia a Remédios" -> "Alergia a Remédios"
                            "Atividades Físicas" -> "Atividades Físicas"
                            "Tatuagem Recente" -> "Tatuagem Recente"
                            else -> key.toString().replaceFirstChar { it.uppercase() }
                        }
                        append("• $titulo: $value\n")
                    }
                }
